...
Claude Code is gaining more and more popularity, so its interface and approach are **starting to be directly cloned**, which is what OpenAI, Atlassian (it works, but keeps deleting everything), and now Google (it freezes for me on authorization - i.e., it doesn't work at all) are trying to do.
